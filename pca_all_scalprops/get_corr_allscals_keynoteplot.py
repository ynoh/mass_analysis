...
def plot_corr(cormat, cc, evals, figname, xlabel="props"):
	...
	if xlabel == "props":
		...
		for i_mprops in xrange(nplot):
			...
			plt.subplots_adjust(hspace=0, bottom=0.3, top=0.6, left=0.2, right=0.9)

			ax.plot(N.arange(n_mprops), cormat[i_mprops, :], 'ro', ms=5.)
			ax.axhline(y=-0.4, xmin=-.5, xmax=n_mprops, linestyle=':')
			ax.axhline(y=0.4, xmin=-.5, xmax=n_mprops, linestyle=':')
			ax.set_ylim(-1., 1.)
			ax.set_xlim(-.5, n_mprops-.5)
			...
			if i_mprops == (nplot)/2:
				...
				ax2.set_xticks(N.arange(n_mprops))
				ax2.set_xticklabels(cc, rotation=90, fontsize=fs['lb1'], position=[0.5, 0.0])
				#plt.setp(ax2.get_xticklabels(), visible=False)
				plt.setp(ax2.get_yticklabels(), visible=False)
			if len(figname) > 0:
				plt.savefig("%s_xaxis%s_keynote.eps" % (figname, xlabel))
